...
**Region-pair preview**:
A short-lived outline of every region pair's capture region and display region, drawn together over the game.
...
The settings-armed state in which one pair's display region accepts mouse drag. Off, the overlay is click-through.
...
The one capture region allowed to trigger voice playback. The operator designates which region pair that capture belongs to; it defaults to the first pair.
...
**OCR cadence**:
The live timing stack that samples frames on one clock, diffs each capture region against its own previous frame, and runs OCR only for regions that changed. OCR is one engine on one queue.
_Avoid_: 识图频率 (that name implies a single Hz); per-pair timer; using this name for the settings-window slider (that lever is OCR interval)
